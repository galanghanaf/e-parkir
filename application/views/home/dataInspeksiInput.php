...
<body id="page-top">
                ...
                <div class="container-fluid mb-5">
                    ...
                    <div class="container-sm">
                        ...
                        <?php foreach ($queryKendaraan as $row) : ?>
                            <form id="formUser" action="<?= base_url() . 'home/dataInspeksiInputAction' ?>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="result" class="form-control" id="inspeksiResult" value="">
                                ...
                                <div class="py-2 mb-5">
                                    <a class="float-left btn font-weight-bold" id="inpeksiMessage"></a>
                                    <button type="submit" class="float-right btn btn-success font-weight-bold mb-5" onclick="submitResultUser(event);">
                                        <i class='fas fa-plus-circle'></i> Simpan Data</button>
    ...
    <script>
        ...
        $('select').change(function() {
            ...
            if (sum == 12) {
                $('#inpeksiMessage').empty().removeClass('btn-warning').removeClass('btn-danger');
                $('#inpeksiMessage').html('Hasil Inpeksi Passed').addClass('btn-success');
                $('#inspeksiResult').val('');
                $('#inspeksiResult').val(sum);
            } else if (sum < 12) {
                $('#inpeksiMessage').empty().removeClass('btn-success').removeClass('btn-danger');
                $('#inpeksiMessage').html('Hasil Inpeksi Warning').addClass('btn-warning');
                $('#inspeksiResult').val('');
                $('#inspeksiResult').val(sum);
            } else if (sum > 12) {
                $('#inpeksiMessage').empty().removeClass('btn-success').removeClass('btn-warning');
